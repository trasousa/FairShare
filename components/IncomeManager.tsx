...
export const IncomeManager: React.FC<IncomeManagerProps> = ({ incomes, currentMonth, users, currency, onAddIncome, onDeleteIncome }) => {
  ...
  const getUserColor = (acc: AccountType) => {
      if (acc === 'SHARED') return users.shared?.color || '#a855f7';
      if (acc === 'USER_1') return users.user_1?.color || '#3b82f6';
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        {(['USER_1', 'USER_2', 'SHARED'] as AccountType[]).map(acc => {
                            const isActive = recipient === acc;
                            const color = getUserColor(acc);
                            const name = acc === 'SHARED' ? 'Shared' : acc === 'USER_1' ? users.user_1.name : users.user_2.name;
                            return (
                                <button 
                                    key={acc}
                                    type="button" 
                                    onClick={() => setRecipient(acc)} 
                                    className={`flex-1 py-2 text-xs rounded-lg border transition font-bold`}
                                    style={{
        ...
        <div className="lg:col-span-2 space-y-4">
             <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                 {(['USER_1', 'USER_2', 'SHARED'] as AccountType[]).map(acc => {
                     ...
                     return (
                         <div key={acc} className="rounded-lg p-3 border border-white/50" style={{ backgroundColor: `${color}10` }}>
                             <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: color }}>{label}</span>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {monthlyIncomes.map(inc => {
                            const color = getUserColor(inc.recipient);
                            return (
                                <tr key={inc.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className="text-xs px-2 py-1 rounded font-bold" style={{ backgroundColor: `${color}15`, color: color }}>
